...
export async function getServerSideProps(context) {
    const post5 = await fetch('https://clblogs-dipr.onrender.com/api/posts')
    ...
    const res = await fetch(`https://clblogs-dipr.onrender.com/api/post/${posturl}`)
    const posts = await res.json()

    const getfeedback = await fetch(`https://clblogs-dipr.onrender.com/api/posts/feed/${posturl}`)
...
const submitFeedback = async (event) => {
    ...
    xhr.open('POST', 'https://clblogs-dipr.onrender.com/api/feedback/');
